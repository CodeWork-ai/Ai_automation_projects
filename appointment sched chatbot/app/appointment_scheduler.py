...
class Notifier:
    ...
    def send_cancellation_confirmation(self, name, time):
...
class Storage:
    ...
    def cancel_appointment(self, name, contact):
        appointments = self.load_appointments()
        updated_appointments = []
        cancelled = None
        
        for appt in appointments:
            if appt['patient_name'] == name and appt['patient_contact'] == contact:
...
class SemanticAppointmentChatbot:
    ...
    def determine_intent(self, text):
        ...
        # If we're in the middle of a cancellation process
        ...
        # Check for rescheduling - FIXED: Added more variations
        ...
        # Check for view appointments first (highest priority)
        if "view my appointment" in text or "view appointments" in text or "my appointments" in text:
            return "VIEW"
    ...
    def update_context(self, text):
        ...
        if self.current_context['expecting_name'] and not entities['name']:
            ...
            if re.match(r'^[A-Z][a-z]+(?:\s+[A-Z][a-z]+)*$', text.strip()):
                self.current_context['name'] = text.strip()
                self.current_context['expecting_name'] = False
        ...
        # FIXED: Always update the intent if it's not UNKNOWN
        # This allows conversational responses to be processed even during rescheduling
        if intent != "UNKNOWN":
    ...
    def handle_booking(self, user_input):
        ...
        if not self.current_context['contact']:
            return f"Thanks, {self.current_context['name']}! What's your email or phone number?"
        ...
        if appointment:
            ...
            # Check if the appointment time is different from the requested time
            original_slot = None
            if patient.availability and appointment.time not in patient.availability:
                original_slot = patient.availability[0]  # Use the first preferred time as the original
            ...
            return f"I'm sorry, but I couldn't find any available slots for your preferred times. Here are our available slots:\n{format_available_slots(self.clinic.available_slots)}"
    ...
    def handle_cancellation(self, user_input, for_reschedule=False):
        ...
        user_appointments = [appt for appt in appointments 
                           if appt['patient_name'] == self.current_context['name'] and appt['patient_contact'] == self.current_context['contact']]
        
        if not user_appointments:
            self.reset_context()
            return f"I couldn't find any appointments for {self.current_context['name']}."
        
        # If there's only one appointment, cancel it directly
        if len(user_appointments) == 1:
            appointment = user_appointments[0]
            cancelled_appointment = self.storage.cancel_appointment(self.current_context['name'], self.current_context['contact'])
            
            if cancelled_appointment:
                self.scheduler.cancel_appointment(cancelled_appointment['time'])
                self.notifier.send_cancellation_confirmation(self.current_context['name'], cancelled_appointment['time'])
